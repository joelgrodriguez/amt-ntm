...
protected branches such as `main` and `master` are never used as Shogun
development bases.
...
4. The worktree agent commits, validates with `npm run build`, then
   ...
   PR. `shogun task report <n>` gives the coordinator the fuller handoff view:
   GitHub, Orca, git, inferred agent roles, reservations, messages, and recent
   local Shogun events. If a Fable terminal is blocked by spend limits, credits,
   auth, or a trust prompt, `task sync` and `task report` warn but do not
   auto-respawn; the coordinator runs `shogun agent fallback <n> --to "claude --model opus --effort xhigh --dangerously-skip-permissions"`.
...
6. You review the running app from `dev`. `shogun task approve <n>`
   ...
   blocking once its last open blocker clears. `shogun task iterate <n>`
...
Exactly one `status:*` label should be present at a time. Shogun creates and
...
Run `shogun doctor` after install to verify gh is authenticated, the configured
repo is reachable, and issue labels are readable.
Use `shogun doctor --agents` after configuring `agentPreflight.commands` in
`.shogun/config.json` to smoke-test headless agent launch commands.
