...
>
> Work on a new branch off `dev`. Follow the worktree agent rules in `AGENTS.md` / `CLAUDE.md`: create a Shogun/GitHub issue, move it to `Processing` before editing, commit clearly, run `shogun task review <n>` when done, and **do not merge into `dev`**. Shogun lands it.
>
