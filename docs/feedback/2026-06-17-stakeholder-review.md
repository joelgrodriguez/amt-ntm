...
- [x] ~~**SSQ3 pricing display:** lead with base price (~$85–90K) + asterisk "trailer sold separately"; stop leading with the$130–143K (trailer-inclusive) range. `app/data/machines/ssq3-multipro.php` + pricing pattern. *(content + theme)*~~ — **done #34** (landed; price_range → "$85K*", note carries "trailer sold separately", schema low_price → 85000; data-driven, fixes all "Starting at" surfaces)
- [ ] **Apply the base-price + disclaimer pattern** to pricing generally (entry vs. souped-up framing), not just Q3. *(content + theme)*
- [x] ~~**Create the NTM trailer article / landing page** — "NTM trailer vs. traditional," engineering + durability story. *(content + theme; Joel ~halfway)*~~ — **done #36 + #38** (landed; live at **`/machines/trailer/`**, hero + bespoke vs-traditional blueprint comparison + 7-reasons engineering + TR23/TR23G models + final CTA; real content from article #7688 + products; db script 028 for the route. #38 fixed a routing collision: the original `/machines/upgrades/trailer/` 2-segment path was hijacked by the Woo product permalink base `/machines/%product_cat%/`, so the page now sits one level under /machines/.)
- [x] ~~**Remove 36% APR** from the Finance Center (and audit all finance percentages for compliance) — `finance-center/faq.php` ("8% to 36% APR") and `corbel.php` (`'8–36%'`). *(content)*~~ — **done #31** (landed; faq → "from 8% APR", corbel → "From 8%"; audit confirmed those were the only two 36% mentions)
- [x] ~~**Fix quote-checklist page copy** — remove the stray "thank you for filling out the form" confirmation text. *(content)*~~ — **done #33** (landed; heading "Thank you for submitting the form." → "Your Machine Quote Checklist", captured as db script 027, applied live. **Note:** page body is still post-download boilerplate + slug ends in `-thank-you` — open content/IA question, see Follow-ups)
- [x] ~~**Remove the which-machine quiz** from the gutter landing page — drop the `which-machine.php` include from `page-seamless-gutter-machines.php`. *(theme)*~~ — **done #32** (landed; kept on `page-machines.php` + `page-roof-wall-panel-machines.php`)
