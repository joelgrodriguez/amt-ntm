    ...
    <div class="container">
        <div class="grid md:grid-cols-2 gap-8 lg:gap-16 items-start">
            ...
            <div class="accessory-gallery">
                <div class="bg-slate-50 rounded overflow-hidden aspect-square flex items-center justify-center p-8">
                    <?php if ($image_id) : ?>
                    ...
                    <?php else : ?>
                        <div class="text-center grid gap-4">
                            ...
                            <span class="text-slate-400 text-sm font-mono"><?php esc_html_e('No image', 'standard'); ?></span>
                        </div>
                    <?php endif; ?>
                </div>
                ...
                <?php if (!empty($gallery_ids)) : ?>
                    <div class="flex gap-2 mt-3 overflow-x-auto">
                        <?php if ($image_id) : ?>
                            <button
                                type="button"
                                class="accessory-gallery__thumb accessory-gallery__thumb--active shrink-0 w-16 h-16 rounded border-2 border-primary overflow-hidden bg-slate-50 cursor-pointer"
                        ...
                        <?php foreach ($gallery_ids as $gid) : ?>
                            <button
                                type="button"
                                class="accessory-gallery__thumb shrink-0 w-16 h-16 rounded border-2 border-slate-200 overflow-hidden bg-slate-50 cursor-pointer hover:border-slate-400 transition-colors"
            ...
            <div class="grid gap-5 content-start">
                <div>
                    ...
                    <h1 id="accessory-title" class="text-2xl font-bold font-mono text-slate-900 md:text-3xl lg:text-4xl">
                        <?php echo esc_html($product->get_name()); ?>
                    </h1>
                </div>

                <?php if (!empty($price_html)) : ?>
                    <p class="text-xl font-semibold text-slate-900"><?php echo wp_kses_post($price_html); ?></p>
                <?php endif; ?>

                <?php if (!empty($excerpt)) : ?>
                    <div class="text-base text-slate-600 leading-relaxed prose prose-sm max-w-none">
                        <?php echo wp_kses_post(wpautop($excerpt)); ?>
                    </div>
                <?php endif; ?>

                <?php if (!empty($sku)) : ?>
                    <p class="text-xs text-slate-400 uppercase tracking-wider font-mono">
    ...
    thumbs.forEach(function(btn) {
        btn.addEventListener('click', function() {
            ...
            thumbs.forEach(function(t) {
                t.classList.remove('accessory-gallery__thumb--active', 'border-primary');
                t.classList.add('border-slate-200');
            });
            btn.classList.add('accessory-gallery__thumb--active', 'border-primary');
